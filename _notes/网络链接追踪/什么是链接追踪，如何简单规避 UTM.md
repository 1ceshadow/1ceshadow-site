[什么是链接追踪，如何简单规避 UTM | 椒盐豆豉](https://blog.douchi.space/url-sanitizer/#gsc.tab=0)
##### September 17, 2023

> 本文总计 2.29k 字, 阅读约需要 5 分钟
...
- 在浏览器输入上面短链接之后，它会被解析成 `https://www.bilibili.com/video/BV1KP411Y7df/?buvid=blah&is_story_h5=false&mid=blah&plat_id=106&share_from=ugc&share_medium=android_i&share_plat=android&share_session_id=blah&share_source=COPY&share_tag=s_i×tamp=1695019227&unique_k=blah&up_id=487640933` 的形式。`?`后面跟着的每个`&`隔开的东西都是一个 tracking 用的参数，此处我把个人信息省去换成了`blah`方便示例。
...
如果读到这里你也想自己制作一款 Chrome 插件的话，[可以参考我这篇简单教程](https://blog.douchi.space/how-to-build-chrome-extension/)。
